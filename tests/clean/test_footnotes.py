"""Выбрасывание не-прозы: подписей к рисункам, сносок, листингов.
...
def test_code_listing_is_dropped():
    # Настоящий блок из Cracking the Coding Interview, доля не-букв 0.32.
    code = (
...
def test_prose_just_below_the_threshold_survives():
    """Порог держит настоящую прозу с цифрами, а не только чистый текст."""
...
def test_short_noisy_string_survives():
    """Короткая строка с цифрами может быть датой или заголовком."""
...
def test_numbered_block_in_normal_font_survives():
    """Нумерованный список это содержание, а не сноска."""
...
def test_small_bottom_block_without_a_number_survives():
    """В Cracking the Coding Interview так набран обычный текст."""
...
def test_clean_book_loses_nothing_to_these_rules():
    """На Кристенсене эти правила не должны срабатывать вообще."""
    from book2audio.extract.pdf import read_pages
